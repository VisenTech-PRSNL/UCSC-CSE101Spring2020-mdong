//-----------------------------------------------------------------------------
// List.c
// source code library for the List ADT
//-----------------------------------------------------------------------------
#include "List.h"
...
long get(List L)
{
    if (length(L) <= 0)
    {
        fprintf(stderr, "USAGE: get() called on empty list\n");
        exit(EXIT_FAILURE);
    }
    if (listIndex(L) < 0)
    {
        fprintf(stderr, "USAGE: get() called on cursor undefined list\n");
        exit(EXIT_FAILURE);
    }
...
{
    Node new;
    if (L != NULL && length(L) > 0 && listIndex(L) > 0)
    {
    ...
    else if (L != NULL && length(L) > 0 && listIndex(L) == 0)
    {
        prepend(L, data);
    }
...
{
    Node new;
    if (L != NULL && length(L) > 0 && listIndex(L) >= 0 && listIndex(L) < length(L) - 1)
    {
        new = malloc(sizeof(NodeObj));
        new->data = data;
    ...
    else if (L != NULL && length(L) > 0 && listIndex(L) == length(L) - 1)
    {
        append(L, data);
    }
...
void delete (List L)
{
    if (L->listLength > 0 && listIndex(L) >= 0)
    {
        if (L->cursor == L->front)
            deleteFront(L);
        if (L->cursor == L->back)
...
void set(List L, long x)
{
    ...
    {
    case -1:
        fprintf(stderr, "set(): Cursor undefined");
        break;
...
void printList(FILE *out, List L)
{
    ...
    {
        fprintf(out, "%ld ", tempCursor->data);
...
List concatList(List A, List B)
{
    ...
    if (target != undefined)
    {
        for (n = 0; n < A->listLength; n++)
        {
            append(new, target->data);
            target = target->next;
        }
        target = B->front;
        for (n = 0; n < B->listLength; n++)
        {
            append(new, target->data);
            target = target->next;
        }
